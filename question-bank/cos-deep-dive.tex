...
\hypertarget{cos-deep-dive-the-priority-file}{%
\section{COS deep-dive --- the priority
file}\label{cos-deep-dive-the-priority-file}}
...
\subsection{Mastery checklist (tick when you can do it
...
\subsection{High-probability questions (have Claude Code grill me on
...
\item
  ...
  log-state \texttt{y} (e.g.~\texttt{y\ =\ ln(S\_T/K)}):

\begin{verbatim}
P(x0,T) = e^{-rT} ∫_R V(y,T) f(y|x0) dy.
\end{verbatim}

  I do not know \texttt{f}, but I know its characteristic function
  \texttt{φ\_\{X\_T\}(t;x0)\ =\ E{[}e\^{}\{i\ t\ X\_T\}\textbar{}x0{]}\ =\ ∫\ e\^{}\{ity\}\ f(y)\ dy},
  i.e.~\texttt{φ} is the Fourier transform of \texttt{f}.

  \textbf{Step 1 --- truncate.} Pick \texttt{{[}a,b{]}} (cumulant rule,
  Q2) containing essentially all the mass and restrict the integral to
  \texttt{{[}a,b{]}}.

  \textbf{Step 2 --- cosine-expand the density.} On \texttt{{[}a,b{]}}
  write the half-range cosine series (prime halves \texttt{k=0}):

\begin{verbatim}
f(y) ≈ Σ'_{k≥0} A_k cos(kπ (y-a)/(b-a)),   A_k = (2/(b-a)) ∫_a^b f(y) cos(kπ(y-a)/(b-a)) dy.
\end{verbatim}

  \textbf{Step 3 --- A\_k from φ (the engine).} Write
  \texttt{cos\ =\ Re\{exp\}}, pull out the phase
  \texttt{e\^{}\{-ikπa/(b-a)\}}, and extend the integral to \texttt{R}
  (the \emph{only} approximation --- range truncation):
...
\end{verbatim}
  ...
  For the call \texttt{V\ =\ K(e\^{}y-1)\^{}+} lives on \texttt{y≥0}, so

\begin{verbatim}
V_k^call = (2/(b-a)) K [ χ_k(0,b) − ψ_k(0,b) ].
\end{verbatim}

  \textbf{Step 5 --- assemble.} Insert the expansion, swap
  \texttt{Σ}/\texttt{∫} (uniform convergence of the cosine series for
  smooth \texttt{f}), and use \texttt{V\_k}:
...
\end{verbatim}
  ...
  inside \texttt{φ(·;x0)}; for BS/Lévy
  \texttt{φ(u;x0)=φ\_lvl(u)\ e\^{}\{iux0\}}, so one set of
  \texttt{\{V\_k\}} prices the whole spot/strike grid (only the cheap
  \texttt{e\^{}\{iux0\}} changes). Cost \texttt{O(N)}, no integration,
  no FFT.
\item
...
\end{verbatim}
  ...
  where \texttt{c1} = mean (centres the box), \texttt{c2} = variance
  (\texttt{√c2} is the natural width scale, so \texttt{L√c2} is an
  ``L-sigma'' box), \texttt{√c4} (fourth cumulant) fattens the box for
  heavy tails / kurtosis (zero for Gaussian), and \texttt{L} is the
  safety multiplier --- larger \texttt{L} lowers the truncation floor
  but needs more terms.
...
\end{verbatim}
  ...
  \textbf{What breaks as T→0:} \texttt{c2\ =\ σ²T\ →\ 0}, so the
  half-width \texttt{L√(c2+√c4)\ →\ 0} and the box collapses toward the
  single point \texttt{c1}. With \texttt{c4=0} (Gaussian) there is no
  \texttt{√c4} floor to keep it open. Two failure modes: (i) the box
  gets too narrow to cover where \texttt{V·f} has mass; (ii) when
  pricing a whole grid of spots/strikes, a range centred by \texttt{c1}
  for one \texttt{(S0,K)} is centred away from other grid points, so the
  cosine reconstruction is evaluated where it is meaningless --- large,
  erratic errors at short maturity.

  \textbf{Diagnostic signature} (this is the tell): the error
  \emph{grows} as \texttt{T→0} and does \emph{not} improve when
  \texttt{N} increases. That distinguishes a \emph{range} error (a fixed
  accuracy floor) from a \emph{series} error (curable by more terms).

  \textbf{Assignment-2 fix:} the truncation range was too narrow at
  short maturities because \texttt{c2} was tiny. I (i) raised \texttt{L}
  at short \texttt{T}, (ii) imposed a minimum half-width floor so
  \texttt{b−a} cannot collapse (equivalently kept a \texttt{√c4}-type
  term), and (iii) made sure the range was computed to cover the entire
  spot/strike grid actually being priced, not a single point. After that
  the error stopped growing as \texttt{T→0} and reverted to exponential
  decay in \texttt{N}.
\item
  ...
  \item
    ...
    of \texttt{f} on \texttt{{[}a,b{]}} kills every sine term, so the
    surviving \texttt{A\_k} are real (\texttt{F\_k} is literally a
    \texttt{Re\{·\}}). A full complex series carries twice the
    coefficients and complex bookkeeping for no gain.
  \item
    \textbf{(ii) Direct sampling from φ.} The cosine coefficient
    \emph{is} the real part of \texttt{φ} evaluated at the grid
    frequency \texttt{kπ/(b-a)}:
  ...
  \item
    \textbf{(iii) Payoff side closes in elementary functions.}
    \texttt{V\_k} are integrals of (polynomial/exponential)×cos, which
    integrate to the closed forms \texttt{χ\_k,\ ψ\_k}. With a
    complex-exponential basis the call coefficients are not as clean.
    The whole method pairs the cosine coefficients of \texttt{f} (from
    \texttt{φ}) against those of \texttt{V} (closed form).
  \end{itemize}
\item
  ...
  \texttt{{[}a,b{]}} --- \emph{fixed} once chosen, it sets the accuracy
  ...
  truncation} at \texttt{N} --- governed by the decay of the cosine
  coefficients \texttt{A\_k}; (3) \textbf{cf/model error} --- feeds
  straight through.

  The rate in \texttt{N} is set by the \textbf{smoothness of
  \texttt{f}}:

  \begin{itemize}
  \tightlist
  \item
    \texttt{f} analytic in a strip / \texttt{f∈C\^{}∞} with integrable
    derivatives ⇒ \texttt{A\_k} decay \textbf{exponentially} ⇒
    \textbf{exponential} convergence in \texttt{N} (machine precision in
    dozens--low hundreds of terms).
  \item
    \texttt{f} with only \texttt{β} integrable derivatives ⇒
    \texttt{A\_k\ =\ O(k\^{}\{-(β+1)\})} ⇒ only \textbf{algebraic}
    convergence. \emph{Lack of smoothness is what kills the exponential
    rate.}
  \end{itemize}

  \textbf{Algebraic case:} a low-regularity density ---
  e.g.~Variance-Gamma with small \texttt{ν} (the density develops a
  non-smooth peak / cusp), or a short-dated density approaching a Dirac,
  or near a jump/kink. There \texttt{A\_k} decay only polynomially and
  you need many more terms.
\item
  Write the Heston characteristic function you used as ground truth.
  State the Feller condition.

  \textbf{A.} Heston: \texttt{dS\ =\ rS\ dt\ +\ √v\ S\ dW1},
  \texttt{dv\ =\ κ(v̄\ −\ v)dt\ +\ η√v\ dW2},
  \texttt{d⟨W1,W2⟩\ =\ ρ\ dt}, with \texttt{κ} mean-reversion speed,
  \texttt{v̄} long-run variance, \texttt{η} vol-of-vol, \texttt{ρ}
  correlation, \texttt{v0} initial variance. The characteristic function
  of the log-price \texttt{x\_T\ =\ ln(S\_T/S0)} (or \texttt{ln\ S\_T})
  is affine-exponential:
...
\end{verbatim}
  ...
  \texttt{D} and \texttt{G} come from solving the Riccati ODE for the
  variance factor of the affine system. (Equivalent ``little-trap'' form
  uses \texttt{G} rather than \texttt{g=1/G} to keep the complex log on
  the principal branch and avoid branch-cut discontinuities --- the cf
  must be evaluated continuously or COS picks up \texttt{φ}-error that
  feeds straight through.)

  \textbf{Feller condition:} \texttt{2κv̄\ ≥\ η²}. It guarantees the CIR
  variance process stays strictly positive (never hits 0), which keeps
  the density well-behaved and \texttt{φ} clean; if violated, \texttt{v}
  can touch zero and the simulation/quadrature near \texttt{v=0} needs
  care.
\item
  ...
  density factorises into one-step transitions, so the
  \texttt{M}-dimensional integral nests into a \textbf{backward
  recursion}. Define continuation and option value at each date:
...
\end{verbatim}
  ...
  The new ingredient: the ``payoff'' \texttt{v(y,t\_m)} is produced by
  the previous step, so its coefficients \texttt{V\_k} are
  \textbf{propagated by backward induction}. Because \texttt{v} is
  piecewise at \texttt{h}, \texttt{V\_k} splits at the barrier:

\begin{verbatim}
V̂_k(t_m) = Ĉ_k(a,h,t_m) + e^{-r(T-t_{m-1})} (2R/(b-a)) Ψ_k(h,b),
\end{verbatim}

  and \texttt{Ĉ\_k} (inserting the COS continuation) becomes
  \texttt{Σ\_j\ φ\_lvl(jπ/(b-a))\ V\_j\ M\_\{k,j\}} with an analytic
  coupling \texttt{M\_\{k,j\}}. \texttt{M\_\{k,j\}} splits into a
  \textbf{Hankel} part (\texttt{j+k}) and a \textbf{Toeplitz} part
  (\texttt{j−k}); Toeplitz/Hankel×vector = convolution = 3 FFTs by
  circulant embedding ⇒ \texttt{O(N\ log\ N)} per step, total
  \texttt{O((M−1)N\ log₂N)}. So --- unlike European COS --- the barrier
  method \emph{does} use the FFT, because it must move the whole
  coefficient vector at every step. ``In'' barriers come from in--out
  parity. Continuous monitoring is the \texttt{M→∞} limit, recovered by
  Richardson extrapolation in \texttt{Δt} using the known
  \texttt{O(√Δt)} discrete-vs-continuous bias. Heston ⇒ 2-D state
  (log-stock, variance): 1-D COS in log-stock + numerical integration
  over variance, or full 2-D COS; Feller \texttt{2κv̄≥η²}.
  ...
  immediate exercise payoff \texttt{g} and the COS continuation value
  \texttt{c(x,t\_m)=e\^{}\{-rΔt\}Σ\textquotesingle{}\_k\ F\_k(x)V\_k(t\_\{m+1\})}.
  The early-exercise boundary \texttt{x*\_m} is the point where
  \texttt{g\ =\ c}; below/above it (call vs put) the option is
  exercised. So each step (i) finds \texttt{x*\_m} (a 1-D root-find on
  \texttt{g\ −\ c\ =\ 0}), (ii) splits the cosine coefficients of
  \texttt{v} at \texttt{x*\_m} into a continuation region (cosine coeffs
  of \texttt{c}, computed from the previous \texttt{V\_k} via the same
  analytic coupling integrals) and an exercise region (cosine coeffs of
  \texttt{g}, closed-form \texttt{χ\_k/ψ\_k}). This is the COS Bermudan
  pricer; the American price is obtained by Richardson extrapolation in
  the number of exercise dates. Structurally identical to the barrier
  recursion, with the moving boundary \texttt{x*\_m} playing the role
  the fixed barrier \texttt{h} plays there.
\item
  Compare COS with Carr--Madan. Advantages and limitations.

  \textbf{A.} Both need only the characteristic function. Carr--Madan
  damps the call (\texttt{c\_T(k)=e\^{}\{αk\}C\_T(k)},
  \texttt{α\textgreater{}0}, to make it \texttt{L¹∩L²}),
  Fourier-transforms it in closed form in terms of \texttt{φ}, and
  recovers the price by an \textbf{inverse FFT} --- i.e.~a
  ...
  \bottomrule\noalign{}
  \endlastfoot
  Core op & one \texttt{O(N)} cosine sum, no integration & FFT
  quadrature, \texttt{O(N\ log\ N)} \\
  Free params & none (range from cumulants) & damping \texttt{α} must be
  tuned \\
  Convergence (smooth \texttt{f}) & exponential in \texttt{N} &
  algebraic (quadrature-limited) \\
  ...
  \texttt{φ} and exponentially-decaying tails --- essentially all
  European pricing under BS, Heston, and standard Lévy/AJD models ---
  because it converges exponentially, needs no free parameter, and gives
  Greeks for free.

  \textbf{Limitations / be careful:} genuinely low-regularity densities
  (slow algebraic \texttt{A\_k} decay), very short maturities, or fat
  tails, where \texttt{{[}a,b{]}} must be chosen carefully (Q2) and
  \texttt{N} raised; and any error in \texttt{φ} (e.g.~a mis-evaluated
  Heston cf across the branch cut) feeds straight through. Carr--Madan's
  FFT also naturally returns a \emph{whole strip of strikes} in one
  transform, which COS matches only because its \texttt{\{V\_k\}} are
  strike-independent in log-moneyness.
\end{enumerate}

\hypertarget{my-answers-concise-board-versions-re-derive-these-out-loud-dont-read}{%
\subsection{My answers (concise board versions --- re-derive these out
loud, don't
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{COS call price.} \texttt{P\ =\ e\^{}\{-rT\}∫V\ f\ dy}; I don't
  know \texttt{f} but know its cf \texttt{φ}. Truncate to
  \texttt{{[}a,b{]}}, cosine-expand \texttt{f}, and the coefficients
  come straight off \texttt{φ}:
  \texttt{A\_k\ ≈\ (2/(b-a))Re\{φ(kπ/(b-a))e\^{}\{-ikπa/(b-a)\}\}}
  (extend the coefficient integral to \texttt{R} --- the only
  approximation). Pair against payoff coeffs
  ...
  The cf enters only as the cosine coefficients of the density;
  \texttt{x0} lives inside \texttt{φ}, so one \texttt{\{V\_k\}} prices
  the whole grid.
\item
  \textbf{\texttt{{[}a,b{]}}.} \texttt{c1\ ±\ L√(c2+√c4)},
  \texttt{L≈10}: \texttt{c1} centres, \texttt{√c2} is the width scale,
  \texttt{√c4} fattens for fat tails, \texttt{L} is the safety margin.
  BS: \texttt{c1=ln(S0/K)+(r-½σ²)T}, \texttt{c2=σ²T}, \texttt{c4=0}, so
  \texttt{{[}a,b{]}=c1±Lσ√T}. As \texttt{T→0}, \texttt{c2→0} so the box
  collapses (no \texttt{√c4} floor in the Gaussian case) → error
  \emph{grows} as \texttt{T→0} and is \emph{not} cured by more
  \texttt{N} (that signature = range, not series, error). Assignment-2
  fix: raise \texttt{L} at short \texttt{T}, impose a minimum half-width
  floor, and size the range to the whole spot/strike grid, not one
  point.
\item
  \textbf{Why cosine.} Even extension ⇒ real \texttt{A\_k}, real
  arithmetic, half the storage; \texttt{A\_k} is literally
  \texttt{Re\{φ\}} at the grid frequency (direct cf sampling); payoff
  coeffs reduce to elementary \texttt{χ\_k,\ ψ\_k}. A complex series
  gives none of these cleanly.
\item
  \textbf{Convergence.} Three errors: range (fixed floor), series (decay
  of \texttt{A\_k}), cf/model (passes through). Rate set by smoothness
  of \texttt{f}: analytic ⇒ exponential \texttt{A\_k} decay ⇒
  exponential in \texttt{N}; only \texttt{β} integrable derivatives ⇒
  \texttt{A\_k=O(k\^{}\{-(β+1)\})} ⇒ algebraic. Algebraic case: VG with
  small \texttt{ν}, or short-dated density near a Dirac/kink.
\item
  \textbf{Heston cf + Feller.} Affine-exponential cf in \texttt{u} with
  \texttt{D=√((κ-iρηu)²+(u²+iu)η²)}, \texttt{G=(κ-iρηu-D)/(κ-iρηu+D)},
  variance term
  \texttt{(v0/η²)((1-e\^{}\{-DT\})/(1-Ge\^{}\{-DT\}))(κ-iρηu-D)} plus
  the \texttt{(κv̄/η²)} drift term; use the ``little-trap'' form for a
  continuous branch. Feller: \texttt{2κv̄\ ≥\ η²} keeps
  \texttt{v\textgreater{}0} and \texttt{φ} clean.
\item
  ...
  \texttt{V\_k} propagated. Barrier: knock out by overwriting
  \texttt{v=R} above \texttt{h}, \texttt{V\_k} splits at \texttt{h}, the
  coupling \texttt{M\_\{k,j\}} is Hankel+Toeplitz ⇒ FFT,
  \texttt{O((M-1)N\ log₂N)}. American: \texttt{v=max(g,c)}, split
  \texttt{V\_k} at the early-exercise boundary \texttt{x*\_m} (root of
  \texttt{g-c}); Bermudan + Richardson in \#exercise dates. The moving
  boundary \texttt{x*} plays the role of the fixed barrier \texttt{h}.
\item
  \textbf{COS vs Carr--Madan.} Both need only \texttt{φ}. CM damps the
  call (param \texttt{α}) and inverts by FFT quadrature --- algebraic,
  \texttt{O(N\ log\ N)}, tunable \texttt{α}. COS is a single
  \texttt{O(N)} cosine sum, no integration, no free parameter,
  exponential convergence, free Greeks. COS wins for smooth densities
  with decaying tails; be careful for low-regularity / very short
  \texttt{T} / fat tails (set \texttt{{[}a,b{]}}, raise \texttt{N}), and
  any \texttt{φ} error feeds straight through.
\end{enumerate}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{lecture-6-drill-set-european-cos}{%
\section{Lecture 6 drill set (European
COS)}\label{lecture-6-drill-set-european-cos}}
...
\subsection{A. Density recovery}\label{a.-density-recovery}}
...
\subsection{B. Why cosine}\label{b.-why-cosine}}
...
\subsection{C. Pricing formula}\label{c.-pricing-formula}}
...
\subsection{D. Payoff coefficients
V\_k}\label{d.-payoff-coefficients-v_k}}
...
\subsection{E. Truncation range (THE
...
\subsection{F. Convergence}\label{f.-convergence}}
...
\subsection{G. Greeks}\label{g.-greeks}}
...
\hypertarget{lecture-10-drill-set-cos-for-barrier-options}{%
\section{Lecture 10 drill set (COS for barrier
options)}\label{lecture-10-drill-set-cos-for-barrier-options}}

Companion to \texttt{notes/Lecture10-notes.pdf}. Same cover-and-answer
format.

\hypertarget{h.-the-pricing-problem}{%
\subsection{H. The pricing problem}\label{h.-the-pricing-problem}}
...
\subsection{I. Reflection principle
...
\subsection{J. PDE routes}\label{j.-pde-routes}}
...
\subsection{K. COS for discrete barriers (the main
...
Key: v is piecewise (R above h, ĉ below) ⇒ V̂\emph{k(t\_m)=Ĉ\_k(a,h,t\_m)
...
Heston's (Lecture 7) and the Feller condition 2κv̄≥η² keeps it
