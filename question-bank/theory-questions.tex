...
\hypertarget{theory-question-bank}{%
\section{Theory question bank}\label{theory-question-bank}}
...
\subsection{Cluster 1 --- Foundations / lattice /
PDE}\label{cluster-1-foundations-lattice-pde}}

\hypertarget{lecture-01}{%
\subsubsection{Lecture 01}\label{lecture-01}}
...
\subsubsection{Lecture 02}\label{lecture-02}}
...
\subsubsection{Lecture 03}\label{lecture-03}}
...
\item
  Globalised Newton: why seed at the price-curve inflection point σ̂?
  What property there makes Newton converge monotonically (no
  overshoot)? \textbf{A.} ∂²C/∂σ² ∝ d\_1d\_2, which vanishes at
  σ̂=√(2\textbar ln(S\_t/K)+rτ\textbar/τ): C is convex for σ\textless σ̂,
  concave for σ\textgreater σ̂, so σ̂ is the inflection point and where
  Vega=C' is \emph{maximal}. Starting there, the contraction ratio
  1−F'(ξ)/F'(σ̂)∈(0,1) because F'(σ̂) is the maximum slope, so the first
...
\subsubsection{Lecture 05}\label{lecture-05}}
...
\subsection{Cluster 2 --- Monte Carlo}\label{cluster-2-monte-carlo}}

\hypertarget{lecture-04}{%
\subsubsection{Lecture 04}\label{lecture-04}}
...
\item
  ...
  Var(X̄\_N)=σ²/N from independence and state the standard error.
  \textbf{A.} Write I=∫h g dx=E\_g{[}h(X){]}, estimate by
  X̄\_N=(1/N)Σh(x\_i), x\_i\textasciitilde g i.i.d. Unbiased:
  E{[}X̄\_N{]}=(1/N)ΣE{[}h(X){]}=I. Variance:
  Var(X̄\_N)=(1/N²)ΣVar(X\_i)=(1/N²)·Nσ²=σ²/N (cross-covariances vanish
...
\item
  ...
  By the CLT (X̄\_N−I)/(σ/√N)→N(0,1), so a (1−α) CI is X̄\emph{N ±
  z}\{1−α/2\}·σ/√N (z=1.96 for 95\%). σ is unknown, so use
  s\_N²=(1/(N−1))Σ(X\_i−X̄\_N)²; the N−1 (Bessel) makes it an
...
\item
  ...
  X̂\emph{\{t\_k\}=X̂}\{t\_\{k−1\}\}+a(t\_\{k−1\},X̂)h+b(t\_\{k−1\},X̂)√h
...
\item
  ...
  E\textbar X̂\_\{Kh\}−X\_T\textbar≤ch\^{}β (pathwise, same Brownian
  path). Weak order β:
  \textbar E{[}f(X̂\_\{Kh\}){]}−E{[}f(X\_T){]}\textbar≤ch\^{}β for smooth
...
\subsection{Cluster 3 --- Fourier / COS}\label{cluster-3-fourier-cos}}
...
\subsubsection{Lecture 6 --- the Fourier family (around
...
\item
  ...
  f̂(ω)=∫f(x)e\^{}\{i2πωx\}dx, f(x)=(1/2π)∫f̂(ω)e\^{}\{−i2πωx\}dω. The
...
\subsubsection{Lecture 10 --- barrier options (theory around
...
\subsection{Cluster 4 --- Models (Heston /
jumps)}\label{cluster-4-models-heston-jumps}}

\hypertarget{lecture-07}{%
\subsubsection{Lecture 07}\label{lecture-07}}
...
\item
  ...
  dS\_t=rS\_t dt+√ν\_t S\_t dW\^{}S\_t, dν\_t=κ(ν̄−ν\_t)dt+γ√ν\_t
  dW\^{}ν\_t, with dW\^{}S dW\^{}ν=ρ dt. Here ν\_t is the instantaneous
  \emph{variance} (vol is √ν\_t), ν̄ the long-run variance,
...
\item
  ...
  independent (W̃\_1,W̃\_2): W\^{}S=W̃\_1, W\^{}ν=ρW̃\_1+√(1−ρ²)W̃\_2, which
...
\item
  ...
  c=2κ/(γ²(1−e\^{}\{−κτ\})), τ=T−t, degrees of freedom d=4κν̄/γ²,
...
\item
  ...
  and what happens to nu\_t when it is violated? \textbf{A.} Feller: 2κν̄
...
\item
  ...
  not to blow up at the origin we need d/2−1≥0, i.e.~d=4κν̄/γ²≥2, which
  is 2κν̄≥γ² --- the Feller condition. A large γ relative to the
  restoring force κν̄ makes excursions to zero likely.
\item
  ...
  becomes K0=(r, κν̄)ᵀ + K1·(x,ν)ᵀ with K1={[}{[}0,−½{]},{[}0,−κ{]}{]}
...
\item
  ...
  gives ∂}\{x\_i\}φ=β\_iφ, ∂\_\{x\_ix\_j\}φ=β\_iβ\_jφ, ∂\_tφ=(α̇+β̇·x)φ.
  Dividing by φ and using that μ, σσᵀ, R are affine in x, collect terms:
  the part \emph{linear} in x must vanish, giving β̇=ρ\_1−K\_1ᵀβ−½βᵀH\_1β
  (the Riccati ODE); the part \emph{constant} in x gives
  α̇=ρ\_0−K\_0·β−½βᵀH\_0β. Boundary conditions β(T,T)=u, α(T,T)=0.
...
\item
  ...
  \textbf{A.} Set g=1/β; then ġ=−β̇/β²=κg−½γ², a linear ODE. With
  ...
  ξ\_κ(t)=(1−e\^{}\{−κt\})/κ. Then α̇=κν̄β integrates (since
  e\^{}\{−κs\}=ξ\_κ') to α(t)=−(2κν̄/γ²)ln(1−½uγ²ξ\_κ(t)). This is the
...
\item
  ...
  α(τ,u)=−rτ+ruτ−κν̄{[}(s+b)τ/γ²+(2/γ²)ln(1−((s+b)/2s)(1−e\^{}\{−sτ\})){]}.
...
\item
  ...
  \textbf{A.} Euler updates ν\_\{i+1\}=ν\_i+κ(ν̄−ν\_i)Δ+γ√ν\_i √Δ Z with
...
\item
  ...
  Recover ∫√ν dW\^{}ν=(1/γ)(ν\_t−ν\_u−κν̄(t−u)+κ∫ν ds) from the CIR
...
\item
  ...
  κ(ν̄−x)f'+½γ²xf'\,' is matched against the squared-Bessel generator
  ...
  discrepancy (κ(ν̄−x) vs n), expressing the CIR integrated-variance
...
\item
  ...
  controls how fast ATM vol converges to √ν̄ in maturity. ν\_0: dominates
  \emph{short}-maturity level. ν̄: dominates \emph{long}-maturity level
  (both mediated by κ). Overall Heston gives heavier log-return tails
  than BS and reproduces the smile.
\end{enumerate}

\hypertarget{lecture-08}{%
\subsubsection{Lecture 08}\label{lecture-08}}
...
\item
  ...
  e\textsuperscript{\{−λt\}/n!)ν̂(u)}n (Y\_k i.i.d.,
  ν̂(u)=E{[}e\^{}\{iuY\}{]}). Pull out e\^{}\{−λt\} and recognise the
  exponential series:
  =e\textsuperscript{\{−λt\}e}\{λtν̂(u)\}=exp(tλ(ν̂(u)−1)).
\item
  ...
  exponent is ψ(u)=λ(ν̂(u)−1)=λ∫(e\^{}\{iuy\}−1)dν(y) --- the
  ...
  satisfy (ν̂(0)=1 just says ν is a probability measure).
\item
...
\item
  ...
  Merton (ν̂(u)=e\^{}\{iuμ\_J−½σ\_J²u²\}, κ=e\^{}\{μ\_J+½σ\_J²\}−1):
  φ(u)=exp(iu(x+(r−½σ²−λκ)τ)−½σ²u²τ+λτ(e\^{}\{iuμ\_J−½σ\_J²u²\}−1)).
  Kou: up-jumps Exp(η\_1) w.p. p, down-jumps −Exp(η\_2) w.p. q=1−p, so
  ν̂(u)=p η\_1/(η\_1−iu)+q η\_2/(η\_2+iu), and
  φ(u)=exp(iu(x+(r−½σ²−λκ)τ)−½σ²u²τ+λτ(ν̂(u)−1)). η\_1 governs the
...
\item
  ...
  θ(c)=∫e\^{}\{c·z\}dν(z) (the ν̂ with complex argument c=β).
\item
  ...
  with β̇=ρ\_1−K\_1ᵀβ−½βᵀH\_1β−l\_1(θ(β)−1),
  α̇=ρ\_0−K\_0·β−½βᵀH\_0β−l\_0(θ(β)−1), boundary β(T,T)=u, α(T,T)=0. The
  ...
  compensator-type term, identical to the (ν̂−1) of the compound-Poisson
  cf.
\item
  ...
  jumps (l\_0=l\_1=0), X=ln S. The ODEs give β̇=0 with β(0)=iu, so β≡iu,
  and α̇=(r−½σ²)β+½σ²β²−r. With β=iu: α(u,τ)=(r−½σ²)iuτ−½σ²u²τ−rτ. Hence
...
\subsection{Cluster 5 --- American
options}\label{cluster-5-american-options}}

\hypertarget{lecture-09}{%
\subsubsection{Lecture 09}\label{lecture-09}}
...
\subsection{Cluster 6 --- Exotics}\label{cluster-6-exotics}}

\hypertarget{lecture-11}{%
\subsubsection{Lecture 11}\label{lecture-11}}
...
\item
  ...
  formula. μ\_G=ln S\_0+(r−q−½σ²)t̄, σ\_G²=(σ²/n²)Σ\_iΣ\_j
  min(t\_i,t\_j). For equally spaced t\_i=iΔt: t̄=(n+1)T/2n,
