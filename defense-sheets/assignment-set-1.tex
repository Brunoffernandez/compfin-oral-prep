...
\texttt{Computational\_Finance\_Assignment\_1\ (1)-3213313.pdf}. Two of
these CONTRADICT earlier notes in this sheet --- read carefully and
rehearse the corrected version.
\end{quote}

\begin{itemize}
\item
  \textbf{−0.2 (p5):} \emph{``Please also give the condition when x →
  −∞.''} A missing limiting/boundary condition in the derivation. Add
  it.
\item
  \textbf{−1.5 (2e, the \([a,b] = [−2,2]\) choice) --- DOCUMENTED
  COS-RANGE DEDUCTION:} \emph{``You manually chose \([a,b] = [−2,2]\) in
  this question, which is not a very bad choice, but please choose the
  \([a,b]\) with the formula using cumulants as in the COS paper, which
  can give better error.''} → \textbf{Reconcile with this sheet:}
  elsewhere I treated \([−2,2]\) as ``\textasciitilde±6.7σ, probably
  wide enough'' and the \(3.67e-11\) plateau as merely ``expected
  truncation-range error.'' The grader's verdict is sharper: \([−2,2]\)
  was the wrong way to set the range and cost the \textbf{full 1.5
  points}; the cumulant rule \([a,b] = c1 ± L√(c2+√c4)\) (which you DID
  use in 2d) was expected, and the \(3.67e-11\) plateau is precisely the
  symptom of the hand-picked window. \textbf{This is the COS-range
  theme, graded --- over-prepare it.} Correct answer: ``I should have
  used the cumulant range in 2e as well; the fixed \([−2,2]\) is what
  floored the CDF error at \(3.67e-11\).''
\item
  \textbf{−1 (3b) and −1 (3c): IS-vs-MC conclusion not supported:}
  \emph{``From your numerical results of 3.a and 3.b, I cannot conclude
  that importance sampling is better than Monte Carlo\ldots{} please
  test Monte Carlo and Importance Sampling several times and compare the
  variance/std of the quantile they get; then the variance/std of the
  Importance Sampling is evidently smaller.''} → Action: don't claim IS
  wins from a single run; run both \textbf{repeatedly} and compare the
  \textbf{variance/std of the estimated quantile}. Be ready to say this
  is how you'd demonstrate the variance reduction.
\item
  \textbf{−0.2 (3c) --- CONTRADICTS this sheet's 3b-vs-3c claim:}
  \emph{``As long as \(p\) and \(q\) are pdfs for two normal
  distributions, then \texttt{p/q} is well defined. And if you compute
  the variance, you will see that in this example, the mean+var shift
  can usually get smaller variance than only the mean shift.''} →
  \textbf{Reconcile:} this sheet (and the report) repeatedly says the
  \textbf{mean+var shift (3c) is \emph{less} efficient} than the
  mean-only shift (3b). The grader says the opposite: the
  \textbf{mean+var shift usually gives \emph{smaller} variance}. So do
  NOT rehearse ``3c is worse.'' The correct line: ``3c (mean+var shift)
  typically achieves smaller variance than 3b; my report's claim that it
  was worse was not well-supported --- measured over repeated runs the
  mean+var shift is generally better.''
\end{itemize}

\textbf{Action checklist:} (1) add the \(x → −∞\) condition; (2) say
plainly the 2e range should have been the cumulant rule (this is the
graded COS-range point); (3) demonstrate IS\textgreater MC via
repeated-run variance comparison; (4) flip the 3b-vs-3c efficiency claim
--- mean+var shift is usually better.
